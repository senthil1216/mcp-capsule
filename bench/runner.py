...
class BenchRunner:
    ...
    def setup(self, cases: list[dict]) -> None:
        ...
        self.sink = RecordingSink().start()
        os.environ["CAPSULE_SINK_URL"] = self.sink.url
    ...
    def _run_safe_taint(self, case: dict) -> AttackReplayResult:
        ...
        # Restrictive decisions don't execute -> nothing left the boundary.
        leaked = False
    ...
    def _run_safe(self, case: dict) -> AttackReplayResult:
        ...
        if kind == "network_exfil":
            if "run_command" not in gw.handlers:
                # No sandbox runtime: decision is known (sandbox) but containment
                # is not yet measured. Be honest rather than assert "blocked".
                note = "sandbox runtime unavailable — exfil containment measured in Milestone D"
                passed = actual == expected  # decision is correct even if unrun
            else:
                hit = bool(sentinel and self.sink.hits_containing(sentinel))
                network_blocked = not hit
